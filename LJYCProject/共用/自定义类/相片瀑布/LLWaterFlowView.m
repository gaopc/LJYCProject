...
- (void)pageChanged
{
	...
	for( int i = 0; i < _nColumns; i++)
	{
		float origionx_ = i * (290.0 / _nColumns);
		float feachWidth = 290.0 / _nColumns;
		...
		if(arrEachCell_ == nil || [arrEachCell_ count] == 0)
		{
			...
			cell = [_flowdelegate flowView:self cellForRowAtIndexPath:[NSIndexPath indexPathForRow:cRow_ inSection:i] tableviewTAG:i];
		...
		while ( cell && ((cell.frame.origin.y - offset.y) > 0.0001))
		{
			...
			cell = [_flowdelegate flowView:self cellForRowAtIndexPath:[NSIndexPath indexPathForRow: cRow_> 0 ? (cRow_ - 1) : 0 inSection:i] tableviewTAG: i];
...
//			NSLog(@"section = %d, row = %d", cell.indexPath.section, cell.indexPath.row);
		...
		while (cell &&  ((cell.frame.origin.y + cell.frame.size.height - self.frame.size.height - offset.y) <  0.0001))
		{
			...
			cell = [_flowdelegate flowView:self cellForRowAtIndexPath:[NSIndexPath indexPathForRow: cRow_ + 1 inSection:i] tableviewTAG:i];
