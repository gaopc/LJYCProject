//
//  ShopOrderBoundPhoneViewController.m
//  LJYCProject
//
...
@interface ShopOrderBoundPhoneViewController ()

@end

@implementation ShopOrderBoundPhoneViewController
...
- (void)viewDidLoad
{
    ...
    [self.view_IOS7 addSubview:[UISubLabel labelWithTitle:@"验证码将发送到原手机上" frame:CGRectMake(10,10,300,30) font:FontSize28 color:FontColor454545 alignment:NSTextAlignmentLeft]];
    ...
    [self.view_IOS7 addSubview:[UISubLabel labelWithTitle:@"原手机号" frame:CGRectMake(10,50,100,30) font:FontSize32 color:FontColor333333 alignment:NSTextAlignmentLeft]];
    self.phoneNumTF = [UISubTextField TextFieldWithFrame:CGRectMake(80,50,200,30) borderStyle:UITextBorderStyleNone textAlignment:NSTextAlignmentLeft placeholder:@"请输入原手机号" font:FontSize32];
	//cuilidong 修改 绑定新手机号

	self.phoneNumTF.text = [NSString stringWithFormat:@"%@", [UserLogin sharedUserInfo]._telephone];
    ...
    UIButton *nextBut = [CoustomButton buttonWithOrangeColor:CGRectMake(20, 155, 280, 45) target:self action:@selector(next:) title:@"下一步"];
    [self.view_IOS7 addSubview:nextBut];
...
-(void)next:(UIButton*)sender
{
    ...
    ASIFormDataRequest * theRequest = [InterfaceClass userDelBinding:[UserLogin sharedUserInfo].userID telephone:self.phoneNumTF.text checkCode:verificationNumTF.text];
...
-(void)onExamineCheckCode:(NSDictionary*)dic
{
    ...
    {
        [self.shopForOrderVC changePhoneNum:@""];
        [UserLogin sharedUserInfo]._telephone = @"";
        ShopOrderBoundNewPhoneViewController *shopOrderBoundNewPhoneVC = [[ShopOrderBoundNewPhoneViewController alloc] init];
        shopOrderBoundNewPhoneVC.shopForOrderVC =self.shopForOrderVC;
        [self.navigationController pushViewController:shopOrderBoundNewPhoneVC animated:YES];
        [shopOrderBoundNewPhoneVC release];
    }
    else
    {
        [UIAlertView alertViewWithMessage:[NSString stringWithFormat:@"%@",[dic objectForKey:@"message"]]];
